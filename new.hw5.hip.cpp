...
__global__ void integrate_kernel(int n, double* qx, double* qy, double* qz, double* vx,
    ...
    // Scaling constants for mixed precision
    constexpr float SCALE = 1e-9f;
    constexpr float SCALE_SQ = SCALE * SCALE;
    ...
    for (int tile = 0; tile < n; tile += blockDim.x) {
        ...
        if (i < n) {
            ...
            for (int j = 0; j < tile_count; ++j) {
                ...
                // Compute subtraction in double for precision
                ...
                // Scale dist2 to fit in float range for rsqrtf
                // We want to compute 1/sqrt(dist2).
                // Let dist2_scaled = dist2 * SCALE_SQ
                // rsqrtf(dist2_scaled) = 1/sqrt(dist2 * SCALE^2) = 1/(sqrt(dist2) * SCALE)
                // So 1/sqrt(dist2) = rsqrtf(dist2_scaled) * SCALE
                
                float dist2_scaled = (float)(dist2 * (double)SCALE_SQ);
                float inv_dist_scaled = rsqrtf(dist2_scaled);
                
                // Convert back to double and unscale
                double inv_dist = (double)inv_dist_scaled * (double)SCALE;
                
                // Newton-Raphson Refinement: y = y * (1.5 - 0.5 * x * y * y)
                // x is dist2, y is inv_dist
                inv_dist = inv_dist * (1.5 - 0.5 * dist2 * inv_dist * inv_dist);
                inv_dist = inv_dist * (1.5 - 0.5 * dist2 * inv_dist * inv_dist);
